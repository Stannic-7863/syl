...
import renderer "../renderer/sdl_gpu"
...
main :: proc() {
	...
	f := renderer.add_font(&r, "./assets/Lora.ttf", 16)

	renderer.default_font = f
	r.clear_color = renderer.color_syl_sdl(BACKGROUND_COLOR)

	syl.create_context(renderer.measure_text)
	...
	run: for {
		...
		renderer.begin(&r)
		renderer.feed_validate(&r, renderer.feed_renderer(&r, ui, 0, nil))
		renderer.render(&r)
	}
}
